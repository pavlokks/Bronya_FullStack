...
function Room() {
  ...
  const handleSubmit = async (e) => {
    ...
    navigate(`/room?address=${encodeURIComponent(filter.address)}`);
  };

  const handleReset = async (e) => {
    e.preventDefault();
    setFilter({ address: '', placetype: 'Room' });
    setFilterData({ address: '', placetype: 'Room' });
    setFilterDatas({ address: '', placetype: 'Room' });
    setCurrentPage(1);
    navigate('/room');
  };
    ...
    if (!islogin) {
      ...
      try {
        ...
        if (data1.success) {
          swal({
            title: 'Готово!',
            text: 'Кімнату додано до списку бажань!',
            icon: 'success',
            button: 'ОК',
          });
          ...
          swal({
            title: 'Вже додано!',
            text: 'Кімната вже є у списку бажань!',
            icon: 'error',
            button: 'ОК',
          });
        }
  ...
  return (
    <div className="filter-section container">
      <div className="filter-header">
        ...
        <p>
          Усього доступно <strong>{totalCount}</strong> кімнат
        </p>
      </div>
      {isLoading ? (
          ...
          {rooms.map((room) => (
            <div
              className={`filter-card ${
                !room.photos || room.photos.length === 0 ? 'no-image' : ''
              }`}
              key={room._id}
            >
              {room.photos && room.photos.length > 0 && (
                <div className="card-image">
                  <img src={room.photos[0]} alt="Кімната" />
                </div>
              )}
              <div className="card-content">
                ...
                <p className="perks">
                  {room.perks &&
                    room.perks.map((perk) => perksTranslations[perk] || perk).join(' • ')}
                </p>
                <p className="owner">
                  Власник: {room.ownername} {room.ownerlast}
                </p>
                <p className="date">
                  Опубліковано: {room.datecreated ? room.datecreated.split('T')[0] : '-'}
                </p>
                <p className="description">{room.description}</p>
              </div>
              <div className="card-actions">
                <div className="price">
                  ₴{room.price * 30} <span>на місяць</span>
                </div>
                ...
                <div className="buttons">
                  ...
                  <button className="btn btn-secondary" onClick={() => addtosaved(room._id)}>
